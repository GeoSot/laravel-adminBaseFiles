aw.forms = aw.forms || {};
aw.forms.fields = aw.forms.fields || {};
aw.forms.fields.Instances = aw.forms.fields.Instances || {};
aw.forms.initializedClass = 'formField-initialized';


aw.forms.fields.imagePicker = function () {
    let classes = {
...
aw.forms.fields.singleDatePicker = function () {
    ...
    function initDateTimeOPicker($el = null) {
        ...
        $(parentSelector).not('.' + aw.forms.initializedClass).each(function (i, elem) {
            if ($(elem).find('input[name$="_formatted"]').first().is(':disabled')) {
                return;
            }
            $(elem).addClass(aw.forms.initializedClass);
    ...
    $(document).on('click', parentSelector + ':not(.' + aw.forms.initializedClass + ')', function () {
...
aw.forms.fields.DateRangePicker = function () {
    ...
    function initDateRangePicker() {
        ...
        $(parentSelector).not('.' + aw.forms.initializedClass).addClass(aw.forms.initializedClass).each(function (i, elem) {
            if ($(elem).find('input[name$="_formatted"]').first().is(':disabled')) {
                return;
            }
            let val = $(elem).find('input').not('*_formatted').first().val();
...
aw.forms.fields.multiplyField = function () {
    $(document).on('click', '.js-addToCollection', function (e) {
        ...
        $itemsContainer.find('textarea.withEditor').each(function () {
            aw.initTextEditor(this);
        });
    });
};


aw.forms.fields.Instances.timePickers = aw.forms.fields.Instances.timePickers || {};
aw.forms.fields.timePicker = function () {
    ...
    let timePicker_OBJ = function (el) {
        ...
        // if (Object.keys(aw.forms.fields.Instances.timePickers).length && aw.forms.fields.Instances.timePickers[name] !== 'undefined') {
        //     return aw.forms.fields.Instances.timePickers[name];
        // } else {
        aw.forms.fields.Instances.timePickers[name] = _this;
        // }
...
aw.forms.fields.colorPicker = function () {
    $('[data-toggle="colorPicker"]').each(function (e) {
        $(this).colorpicker();
    });
};
aw.forms.fields.sortable = function () {
...
aw.forms.fields.removeParent = function () {
    $(document).on('click', '[data-toggle="removeParent"]', function (e) {
        $(this).closest($(this).data('target')).remove();
    });
};

aw.forms.fields.Init = function () {
    aw.forms.fields.singleDatePicker();
    aw.forms.fields.DateRangePicker();
    aw.forms.fields.imagePicker();
    aw.forms.fields.multiplyField();
    aw.forms.fields.timePicker();
    aw.forms.fields.colorPicker();
    aw.forms.fields.removeParent();
    aw.forms.fields.sortable();
};

aw.forms.fields.Init();