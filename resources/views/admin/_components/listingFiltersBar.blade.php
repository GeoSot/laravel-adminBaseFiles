...
@if(!empty( request()->query()))
    <div class="mb-3">
            <a href="{{route($viewVals->get('baseRoute').'.index','clean-queries')}}" class="btn btn-outline-info  ">
                <span class=""> @lang($packageVariables->get('nameSpace')."{$lang}.listFilters.cleanFilters")</span>
...
<form action="{{route($viewVals->get('baseRoute').'.index')}}" id="tableForm" class="main_form row mb-3 @] " method="GET">
    ...
    @if(!empty( $viewVals->get('extraValues')->get('searchable')))
        <div class="form-group   col-auto">
                <div class="input-group ">
                    <input id="keyword" type="text" class="form-control" placeholder=" @lang($packageVariables->get('nameSpace')."{$lang}.listFilters.search")" name="keyword"
    ...
    @if(in_array('enabled',$viewVals->get('options')->get('fillable')) and in_array('enabled',$viewVals->get('extraValues')->get('listable')))
        <div class="form-group  col-auto">
                    <select class="form-control  custom-select " name="status" id="status" data-change="js-submit-form">
                        <option value=""> @lang($packageVariables->get('nameSpace')."{$lang}.listFilters.selectStatus")</option>
                        <option {{ ($params['status']==1) ? 'selected': '' }} value="1"> @lang($packageVariables->get('nameSpace')."{$lang}.listFilters.statusEnabled")</option>
                        <option {{ ($params['status']==-1) ? 'selected': '' }} value="-1"> @lang($packageVariables->get('nameSpace')."{$lang}.listFilters.statusDisabled")</option>
                    </select>
                </div>
    @endif
    <div class="form-group ml-auto col-auto ">
                <select class="form-control  custom-select " name="num_of_items" data-change="js-submit-form">
                    @php($sessionNum=Session::get('num_of_items', \GeoSot\BaseAdmin\Facades\Settings::get('admin.generic.paginationDefaultNumOfItems',100)))
                    @foreach(\GeoSot\BaseAdmin\Facades\Settings::get('admin.generic.paginationOptions',[10, 25, 50,100]) as $num)
