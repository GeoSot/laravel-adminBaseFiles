...
@endphp
@if($filters->count())
    <div class="px-5"> <hr class=" mt-2 mb-1"></div>
    <div class="text-right">
        <button class="btn btn-outline-secondary btn-sm" type="button" data-toggle="collapse" data-target="#extraFilters_collapse" aria-expanded="false"
             ...
             @lang($packageVariables->get('nameSpace').$lang.'.button.extraFilters')
        </button>
    </div>

      <div class="collapse {{($requestHasExtraFilters or \GeoSot\BaseAdmin\Facades\Settings::get('admin.generic.keepExtraFiltersOpen',false))? 'show':''}}" id="extraFilters_collapse">

   <div class="row">
    @foreach($filters as  $name=>$filter)
           @php($filterType=\Illuminate\Support\Arr::get($filter,'type'))
           @if( in_array($filterType,['boolean','bool','hasValueInside']))
               <div class="form-group   input-group-sm d-inline-block  col-auto  ">
                   <label for="extra_filters[{{$name}}]" class="small control-label"> @lang($packageVariables->get('nameSpace').$modelLang.'.filters.'.$name)</label>
                   <select class="form-control custom-select-sm custom-select" name="extra_filters[{{$name}}]">
                       <option value=""> @lang($packageVariables->get('nameSpace').$lang.'.formTitles.formSelectPlaceHolder')</option>
                       @foreach(['true'=>1,'false'=>0] as $key=>$val)
                           <option @if(in_array($val,(array)Arr::get($requestParams,$name)))selected="selected" @endif value="{{$val}}"> @lang($packageVariables->get('nameSpace')."$lang.listFilters.$key")</option>
                       @endforeach
                   </select>
               </div>
           @endif
           ...
           @if(in_array($filterType,['multiSelect','select']))
               ...
               <div class="form-group   input-group-sm d-inline-block  col-auto  ">
                   <label for="extra_filters[{{$name}}]" class="small control-label"> @lang($packageVariables->get('nameSpace').$modelLang.'.fields.'.$name)</label>
                   <select class="form-control custom-select-sm custom-select" name="extra_filters[{{$name}}]@if($isMultiSelect)[]@endif" @if($isMultiSelect) multiple @endif>
                       @if((\Illuminate\Support\Arr::get($filter,'type') =='select'))
                           <option value=""></option>
                       @endif
                       @foreach(\Illuminate\Support\Arr::get($filter,'values',[]) as $key=> $val)
                           <option @if(in_array($key,(array)\Illuminate\Support\Arr::get($requestParams,$name)))selected="selected" @endif value="{{$key}}">{{$val}}</option>
                       @endforeach
                   </select>
               </div>
           @endif
               ...
               <div class="form-group   d-inline-block  col-auto ">
                   ...
                   <div data-toggle="calendar" data-name="extra_filters[{{$name}}]" data-locale="DD/MM/YYYY" class="input-group  input-group-sm">
                       <div class="flex-fill input-group-sm">
                           <input name="extra_filters[{{$name}}]" type="hidden" value="{{\Illuminate\Support\Arr::get($requestParams,$name)}}" id="{{$name}}">
                           <input readonly="readonly" name="extra_filters[{{$name}}_formatted]" type="text" class="form-control bg-light"
                                  value="{{optional(\Illuminate\Support\Arr::get($requestParams,$name))->format('d/m/Y') ?? ''}}">
                       </div>
           ...
           @if(in_array($filterType,['dateRange']))
               @php($hasValues=!empty(\Illuminate\Support\Arr::get($requestParams,$name.'.start').\Illuminate\Support\Arr::get($requestParams,$name.'.end')))
               @php($formattedValue=optional(\Illuminate\Support\Arr::get($requestParams,$name.'.start'))->format('d/m/Y') .' - ' .optional(\Illuminate\Support\Arr::get($requestParams,$name.'.end'))->format('d/m/Y'))
               <div class="form-group   d-inline-block  col-auto ">
                 <div class="d-flex justify-content-between">
                   ...
                   <div data-toggle="dateRangeCalendar" data-name="extra_filters[{{$name}}]" data-locale="DD/MM/YYYY" class="input-group  input-group-sm">
                       <div class="flex-fill input-group-sm">
                           <input name="extra_filters[{{$name}}][start]" type="hidden" value="{{\Illuminate\Support\Arr::get($requestParams,$name.'.start')}}" id="{{$name}}_start">
                           <input name="extra_filters[{{$name}}][end]" type="hidden" value="{{\Illuminate\Support\Arr::get($requestParams,$name.'.end')}}" id="{{$name}}_end">
