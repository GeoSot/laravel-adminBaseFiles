...
<div class="card">
    ...
    <p>Warning, translations are not visible until they are exported back to the app/lang file, using <code>php artisan translation:export</code> command or publish button.</p>
        ...
        @if(!isset($group))
            ...
            <form class="form-find" method="POST" action="{!! action('\Barryvdh\TranslationManager\Controller@postFind') !!}" data-remote="true" role="form"
        ...
        @endif
        @isset($group)
            <form class="form-inline form-publish" method="POST" action="{!! action('\Barryvdh\TranslationManager\Controller@postPublish', $group)  !!}" data-remote="true"
                ...
                <a href="{{action('\Barryvdh\TranslationManager\Controller@getIndex') }}" class="btn btn-secondary">Back</a>
            </div>
            </form>
        @endif
        ...
        <div class="form-group">
            <input type="submit" class="btn " name="add-group" value="Add and edit keys"/>
        </div>
    </form>
        @if($group)
